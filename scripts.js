...
app.controller('leafletController', ['$scope', function leafletController($scope) {
    ...
    var _getBorderingPoints = function (center,  currentStop, clonedArray, j) {
        ...
        var test = _calcLatLngDistance(currentStop.lat, currentStop.lon, clonedArray[j + index].lat, clonedArray[j + index].lon);
        while (((j + index) < clonedArray.length-1) && (distance > test)) {  //get going until we are over 40 meters or we hit the last item in the array
